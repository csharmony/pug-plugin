...
public void OnPluginStart()
{
	...
	/** ConVars **/
	g_AdminFlagCvar = CreateConVar("sm_pp_admin_flag", "b", "Admin flag to mark players as having elevated permissions - e.g. can always pause,setup,end the game, etc.");
	g_AllowCustomReadyMessageCvar = CreateConVar("sm_pp_allow_custom_ready_messages", "1", "Whether users can set custom ready messages saved via a clientprefs cookie");
	g_AnnounceCountdownCvar = CreateConVar("sm_pp_announce_countdown_timer", "1", "Whether to announce how long the countdown has left before the lo3 begins.");
	g_AutoRandomizeCaptainsCvar = CreateConVar("sm_pp_auto_randomize_captains", "0", "When games are using captains, should they be automatically randomized once? Note you can still manually set them or use .rand/!rand to redo the randomization.");
	g_AutoSetupCvar = CreateConVar("sm_pp_autosetup", "1", "Whether a pug is automatically setup using the default setup options or not.");
	g_DemoNameFormatCvar = CreateConVar("sm_pp_demo_name_format", "pug_{TIME}_{MAP}", "Naming scheme for demos. You may use {MAP}, {TIME}, and {TEAMSIZE}. Make sure there are no spaces or colons in this.");
	g_DemoTimeFormatCvar = CreateConVar("sm_pp_time_format", "%Y-%m-%d_%H%M", "Time format to use when creating demo file names. Don't tweak this unless you know what you're doing! Avoid using spaces or colons.");
	g_DisplayMapVotesCvar = CreateConVar("sm_pp_display_map_votes", "1", "Whether votes cast by players will be displayed to everyone");
	g_DoVoteForKnifeRoundDecisionCvar = CreateConVar("sm_pp_vote_for_knife_round_decision", "1", "If 0, the first player to type .stay/.swap/.t/.ct will decide the round round winner decision - otherwise a majority vote will be used");
	g_EchoReadyMessagesCvar = CreateConVar("sm_pp_echo_ready_messages", "1", "Whether to print to chat when clients ready/unready.");
	g_ExcludedMaps = CreateConVar("sm_pp_excluded_maps", "0", "Number of past maps to exclude from map votes. Setting this to 0 disables this feature.");
	g_ExcludeSpectatorsCvar = CreateConVar("sm_pp_exclude_spectators", "1", "Whether to exclude spectators in the ready-up counts. Setting this to 1 will exclude specators from being selected by captains as well.");
	g_ExecDefaultConfigCvar = CreateConVar("sm_pp_exec_default_game_config", "1", "Whether gamemode_competitive (the matchmaking config) should be executed before the live config.");
	g_ForceDefaultsCvar = CreateConVar("sm_pp_force_defaults", "0", "Whether the default setup options are forced as the setup options (note that admins can override them still).");
	g_InstantRunoffVotingCvar = CreateConVar("sm_pp_instant_runoff_voting", "1", "If set, map votes will run instant-runoff style where each client selects their top 3 maps in preference order.");
	g_KnifeConfigCvar = CreateConVar("sm_pp_knife_cfg", "sourcemod/pug-plugin/knife.cfg", "Config to execute when the knife round begins. CVars set in this file are saved before execution, and reverted back to their pre-knife-config values when the game goes live, before executing the live.cfg.");
	g_LiveCfgCvar = CreateConVar("sm_pp_live_cfg", "sourcemod/pug-plugin/live.cfg", "Config to execute when the game goes live");
	g_MapListCvar = CreateConVar("sm_pp_maplist", "maps.txt", "Maplist file in addons/sourcemod/configs/pug-plugin to use. You may also use a workshop collection ID instead of a maplist if you have the SteamWorks extension installed.");
	g_MapVoteTimeCvar = CreateConVar("sm_pp_mapvote_time", "30", "How long the map vote should last if using map-votes.", _, true, 10.0);
	g_MaxTeamSizeCvar = CreateConVar("sm_pp_max_team_size", "5", "Maximum size of a team when selecting team sizes.", _, true, 2.0);
	g_MessagePrefixCvar = CreateConVar("sm_pp_message_prefix", "[{YELLOW}PUG{NORMAL}]", "The tag applied before plugin messages. If you want no tag, you can set an empty string here. Note that beginning the string with a color will not render that color - you can workaround with a space or another character first.");
	g_MutualUnpauseCvar = CreateConVar("sm_pp_mutual_unpausing", "1", "Whether an unpause command requires someone from both teams to fully unpause the match. Note that this forces the pause/unpause commands to be unrestricted (so anyone can use them).");
	g_PausingEnabledCvar = CreateConVar("sm_pp_pausing_enabled", "0", "Whether pausing is allowed.");
	g_PostGameCfgCvar = CreateConVar("sm_pp_postgame_cfg", "sourcemod/pug-plugin/warmup.cfg", "Config to execute after games finish; should be in the csgo/cfg directory.");
	g_QuickRestartsCvar = CreateConVar("sm_pp_quick_restarts", "0", "If set to 1, going live won't restart 3 times and will just do a single restart.");
	g_RandomizeMapOrderCvar = CreateConVar("sm_pp_randomize_maps", "1", "When maps are shown in the map vote/veto, whether their order is randomized.");
	g_RandomOptionInMapVoteCvar = CreateConVar("sm_pp_random_map_vote_option", "1", "Whether option 1 in a mapvote is the random map choice.");
	g_ReadyToStartCvar = CreateConVar("sm_pp_ready_to_start", "4", "Number of ready players required to start the match. Set to 0 to use twice the team size (2 * players per team).", _, true, 0.0);
	g_SetupEnabledCvar = CreateConVar("sm_pp_setup_enabled", "1", "Whether the sm_setup commands are enabled");
	g_SnakeCaptainsCvar = CreateConVar("sm_pp_snake_captain_picks", "0", "If set to 0: captains pick players in a ABABABAB order. If set to 1, in a ABBAABBA order. If set to 2, in a ABBABABA order. If set to 3, in a ABBABAAB order.");
	g_StartDelayCvar = CreateConVar("sm_pp_start_delay", "5", "How many seconds of a countdown phase right before the lo3 process begins.", _, true, 0.0, true, 60.0);
	g_UseGameWarmupCvar = CreateConVar("sm_pp_use_game_warmup", "1", "Whether to use csgo's built-in warmup functionality. The warmup config (sm_pp_warmup_cfg) will be executed regardless of this setting.");
	g_WarmupCfgCvar = CreateConVar("sm_pp_warmup_cfg", "sourcemod/pug-plugin/warmup.cfg", "Config file to run before/after games; should be in the csgo/cfg directory.");
	g_WarmupMoneyOnSpawnCvar = CreateConVar("sm_pp_money_on_warmup_spawn", "1", "Whether clients recieve 16,000 dollars when they spawn. It's recommended you use mp_death_drop_gun 0 in your warmup config if you use this.");

	/** Create and exec plugin's configuration file **/
	AutoExecConfig(true, "pp", "sourcemod/pug-plugin");

	g_CvarVersionCvar = CreateConVar("sm_pp_version", PLUGIN_VERSION, "Current pug-plugin version", FCVAR_SPONLY | FCVAR_REPLICATED | FCVAR_NOTIFY | FCVAR_DONTRECORD);
	...
	AddPugPluginCommand("ready", Command_Ready, "Marks the client as ready", Permission_All, ChatAlias_WhenSetup);
	AddPugPluginCommand("notready", Command_NotReady, "Marks the client as not ready", Permission_All, ChatAlias_WhenSetup);
	AddPugPluginCommand("setup", Command_Setup, "Starts pug setup (.ready, .capt commands become avaliable)", Permission_All);
	AddPugPluginCommand("rand", Command_Rand, "Sets random captains", Permission_Captains, ChatAlias_WhenSetup);
	AddPugPluginCommand("pause", Command_Pause, "Pauses the game", Permission_All, ChatAlias_WhenSetup);
	AddPugPluginCommand("unpause", Command_Unpause, "Unpauses the game", Permission_All, ChatAlias_WhenSetup);
	AddPugPluginCommand("endgame", Command_EndGame, "Pre-emptively ends the match", Permission_Leader);
	AddPugPluginCommand("forceend", Command_ForceEnd, "Pre-emptively ends the match, without any confirmation menu", Permission_Leader);
	AddPugPluginCommand("forceready", Command_ForceReady, "Force-readies a player", Permission_Admin, ChatAlias_WhenSetup);
	AddPugPluginCommand("leader", Command_Leader, "Sets the pug leader", Permission_Leader);
	AddPugPluginCommand("capt", Command_Capt, "Gives the client a menu to pick captains", Permission_Leader);
	AddPugPluginCommand("stay", Command_Stay, "Elects to stay on the current team after winning a knife round", Permission_All, ChatAlias_WhenSetup);
	AddPugPluginCommand("swap", Command_Swap, "Elects to swap the current teams after winning a knife round", Permission_All, ChatAlias_WhenSetup);
	AddPugPluginCommand("t", Command_T, "Elects to start on T side after winning a knife round", Permission_All, ChatAlias_WhenSetup);
	AddPugPluginCommand("ct", Command_Ct, "Elects to start on CT side after winning a knife round", Permission_All, ChatAlias_WhenSetup);
	AddPugPluginCommand("forcestart", Command_ForceStart, "Force starts the game", Permission_Admin, ChatAlias_WhenSetup);
	AddPugPluginCommand("addmap", Command_AddMap, "Adds a map to the current maplist", Permission_Admin);
	AddPugPluginCommand("removemap", Command_RemoveMap, "Removes a map to the current maplist", Permission_Admin);
	AddPugPluginCommand("listpugmaps", Command_ListPugMaps, "Lists the current maplist", Permission_All);
	AddPugPluginCommand("start", Command_Start, "Starts the game if autolive is disabled", Permission_Leader, ChatAlias_WhenSetup);
	AddPugPluginCommand("addalias", Command_AddAlias, "Adds a pug-plugin alias, and saves it to the chatalias.cfg file", Permission_Admin);
	AddPugPluginCommand("removealias", Command_RemoveAlias, "Removes a pug-plugin alias", Permission_Admin);
	AddPugPluginCommand("setdefault", Command_SetDefault, "Sets a default setup option", Permission_Admin);
	AddPugPluginCommand("setdisplay", Command_SetDisplay, "Sets whether a setup option will be displayed", Permission_Admin);
	AddPugPluginCommand("readymessage", Command_ReadyMessage, "Sets your ready message", Permission_All);
	LoadExtraAliases();
	...
	g_hOnHelpCommand = CreateGlobalForward("PugPlugin_OnHelpCommand", ET_Ignore, Param_Cell, Param_Cell, Param_Cell, Param_CellByRef);
	...
	g_hOnPlayerAddedToCaptainMenu = CreateGlobalForward("PugPlugin_OnPlayerAddedToCaptainMenu", ET_Ignore, Param_Cell, Param_Cell, Param_String, Param_Cell);
	...
	g_hOnSetup = CreateGlobalForward("PugPlugin_OnSetup", ET_Ignore, Param_Cell, Param_Cell, Param_Cell, Param_Cell);
	g_hOnSetupMenuOpen = CreateGlobalForward("PugPlugin_OnSetupMenuOpen", ET_Event, Param_Cell, Param_Cell, Param_Cell);
	g_hOnSetupMenuSelect = CreateGlobalForward("PugPlugin_OnSetupMenuSelect", ET_Ignore, Param_Cell, Param_Cell, Param_String, Param_Cell);
	g_hOnStartRecording = CreateGlobalForward("PugPlugin_OnStartRecording", ET_Ignore, Param_String);
	g_hOnStateChange = CreateGlobalForward("PugPlugin_OnGameStateChanged", ET_Ignore, Param_Cell, Param_Cell);
	...
	g_ReadyMessageCookie = RegClientCookie("pp_ready", "Pug-plugin ready message", CookieAccess_Protected);
...
static void AddPugPluginCommand(const char[] command, ConCmd callback, const char[] description, Permission p, ChatAliasMode mode = ChatAlias_Always)
{
	...
	if(readyPlayers >= GetReadyPlayersRequired() || g_ForceStartSignal)
	{
		...
		if(g_OnDecidedMap)
		{
			if(g_TeamType == TeamType_Captains)
			{
				...
				{
					g_LiveTimerRunning = false;
					PrintHintTextToAll("%t\n%t", "ReadyStatusPlayers", readyPlayers, GetReadyPlayersRequired(), "ReadyStatusAllReadyPick");
			...
			else
			{
				...
				if(g_AutoLive)
				{
					PrintHintTextToAll("%t\n%t", "ReadyStatusPlayers", readyPlayers, GetReadyPlayersRequired(), "ReadyStatusAllReady");
				}
				else
				{
					PrintHintTextToAll("%t\n%t", "ReadyStatusPlayers", readyPlayers, GetReadyPlayersRequired(), "ReadyStatusAllReadyWaiting");
				}
		...
		else
		{
			if(g_MapType == MapType_Veto)
			{
				...
				{
					g_LiveTimerRunning = false;
					PrintHintTextToAll("%t\n%t", "ReadyStatusPlayers", readyPlayers, GetReadyPlayersRequired(), "ReadyStatusAllReadyVeto");
			...
			{
				g_LiveTimerRunning = false;
				PrintHintTextToAll("%t\n%t", "ReadyStatusPlayers", readyPlayers, GetReadyPlayersRequired(), "ReadyStatusAllReadyVote");
...
public void StatusHint(int readyPlayers, int totalPlayers)
{
	...
	bool captainsNeeded = (!g_OnDecidedMap && g_MapType == MapType_Veto) || (g_OnDecidedMap && g_TeamType == TeamType_Captains);
...
// if we did the setup command from the console just use the default settings
		ReadSetupOptions();
		PugPlugin_SetupGame(g_TeamType, g_MapType, g_PlayersPerTeam, g_RecordGameOption, g_DoKnifeRound, g_AutoLive);
	}
	else
	{
...
static bool CheckChatAlias(const char[] alias, const char[] command, const char[] chatCommand, const char[] chatArgs, int client, ChatAliasMode mode)
{
...
public void PrintSetupInfo(int client)
{
	...
	if(g_DisplayTeamSize || g_DisplayTeamType)
	{
		...
		PugPlugin_Message(client, "%t: ({GREEN}%d vs %d{NORMAL}) {GREEN}%s", "TeamTypeOption", g_PlayersPerTeam, g_PlayersPerTeam, buffer);
	}
