...
static char g_SetupCoercions[][][] = { { "map", "maptype" }, { "teams", "teamtype" }, { "team", "teamtype" }, { "knife", "kniferound" }, { "autolo3", "autolive" }, { "demo", "record" }, { "changemap", "mapchange" },  };
...
stock bool CheckSetupOptionValidity(int client, char[] setting, const char[] value, bool setDefault = true, bool setDisplay = false)
{
	...
	{
		if(setDefault && !StrEqual(value, "vote") && !StrEqual(value, "veto") && !StrEqual(value, "current"))
		{
			PugPlugin_Message(client, "%s is not a valid option for setting %s, valid options are vote, veto, current", value, setting);
			return false;
		}
		else if(setDefault)
		{
	...
	else if(StrEqual(setting, "teamtype", false))
	{
		...
		{
			PugPlugin_Message(client, "%s is not a valid option for setting %s, valid options are captains, manual, random", value, setting);
			return false;
		}
		else if(setDefault)
		{
